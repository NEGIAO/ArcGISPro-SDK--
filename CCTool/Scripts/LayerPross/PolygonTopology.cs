        ...
        protected override async void OnClick()
        {
            try
            {
                ...
                await QueuedTask.Run(() =>
                {
                    ...
                    pw.AddMessageMiddle(10, "创建拓扑并设置规则");
                    // 新建拓扑
                    ...
                    pw.AddMessageMiddle(20, "生成重叠错误");
                    // 验证拓扑
                    Arcpy.ValidateTopology(top_path);
                    // 输出TOP错误
                    Arcpy.ExportTopologyErrors(top_path, gdb, "TopErr");

                    pw.AddMessageMiddle(20, "生成空隙错误");
                    // 生成空隙
                    ComboTool.GetCave(fc_path, gdb + @"\" + err_fc);
                    // 添加说明字段
                    ...
                    // 加载错误面图层
                    MapCtlTool.AddLayerToMap(gdb + @"\" + err_fc);

                    pw.AddMessageMiddle(20, "生成错误标记");

                    // 重叠错误说明赋值
                    FeatureLayer init_layer = map.FindLayers(err_fc)[0] as FeatureLayer;
                    using (Table table = init_layer.GetTable())
                    {
                    ...
                    pw.AddMessageMiddle(20, "应用错误图层的显示符号");

                    // 复制图层符号
                    string copy_lyrx = def_path + @"\检查结果.lyrx";
                    DirTool.CopyResourceFile(@"CCTool.Data.Layers." + @"检查结果.lyrx", copy_lyrx);
                    // 应用图层符号
                    Arcpy.ApplySymbologyFromLayer(err_fc, copy_lyrx);

                    // 如果拓扑结果有线错误的话，就加载
                    string lineErr = $@"{gdb}\TopErr_line";
                    if (lineErr.GetFeatureCount() != 0)
                    {
                        MapCtlTool.AddLayerToMap(lineErr);
                    }

                    // 删除中间要素
                    List<string> list_del = new List<string>() { "TopErr_point", "TopErr_poly" };
