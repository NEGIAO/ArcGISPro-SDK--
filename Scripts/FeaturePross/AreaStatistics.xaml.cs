...
namespace CCTool.Scripts.UI.ProWindow
{
    ...
    public partial class AreaStatistics : ArcGIS.Desktop.Framework.Controls.ProWindow
    {
        ...
        public static List<int> GetDigit(string digitText)
        {
        ...
        {
            List<int> digit = GetDigit(combox_digit.Text);
            // 当选择更改时，自动刷新面积计算
            InitiArea(digit[0], digit[1]);
        }



        // 统计面积
        private async void InitiArea(int digit1, int digit2)
        {
            ...
            try
            {
                await QueuedTask.Run(() =>
                {
                    ...
                    hectares = Math.Round(AreaUnit.SquareMeters.ConvertTo(polygonArea, AreaUnit.Hectares), digit2);
                    // 将面积转换为平方公里并四舍五入保留 4 位小数
                    squKilometers = Math.Round(AreaUnit.SquareMeters.ConvertTo(polygonArea, AreaUnit.SquareKilometers), digit2);
                    // 将面积转换为 'mu' 并四舍五入保留 4 位小数
                    areaMuValue = Math.Round(AreaUnit.SquareMeters.ConvertTo(polygonArea, areaMu), digit2);


                    if (has_geo)       // 如果都有正确的坐标系，就计算椭球面积
                    {
                        geo_hectares = Math.Round(AreaUnit.SquareMeters.ConvertTo(geoArea, AreaUnit.Hectares), digit2);
                        geo_squKilometers = Math.Round(AreaUnit.SquareMeters.ConvertTo(geoArea, AreaUnit.SquareKilometers), digit2);
                        geo_areaMuValue = Math.Round(AreaUnit.SquareMeters.ConvertTo(geoArea, areaMu), digit2);
                    }
                });

                // 显示结果
                text_area_squ.Text = Math.Round(polygonArea, digit1).RoundWithFill(digit1);
                text_area_ha.Text = hectares.RoundWithFill(digit2);
                text_area_km.Text = squKilometers.RoundWithFill(digit2);
                text_area_mu.Text = areaMuValue.RoundWithFill(digit2);
                // 默认先隐藏椭球面积的信息
                ...
                if (has_geo)
                {
                    text_geoarea_squ.Text = Math.Round(geoArea, digit1).RoundWithFill(digit1);
                    text_geoarea_ha.Text = geo_hectares.RoundWithFill(digit2);
                    text_geoarea_km.Text = geo_squKilometers.RoundWithFill(digit2);
                    text_geoarea_mu.Text = geo_areaMuValue.RoundWithFill(digit2);
                    // 显示椭球面积的信息
