        ...
        private async void btn_go_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                await QueuedTask.Run(() =>
                {
                    ...
                    for (int i = 1; i < 5; i++)
                    {
                        pw.AddMessageMiddle(20, $"第{i}轮汇总指标", Brushes.Gray);
                        // 村庄功能映射
                        ...
                        // 属性映射现状用地
                        ExcelTool.AttributeMapperDouble(excel_sheet, 14, 8, dic, 5);
                        // 属性映射规划用地
                        ExcelTool.AttributeMapperDouble(excel_sheet, 14, 10, dic_gh, 5);
                    }

                    // 删除0值行
                    ExcelTool.DeleteNullRow(excel_sheet, new List<int>() { 8, 10 }, 5);
                    // 删除指定列
                    ExcelTool.DeleteCol(excel_sheet, new List<int>() { 14 });
                    // 改Excel中的单位
                    ExcelTool.WriteCell(excel_path, 2, 1, $"单位：{unit}、%");
